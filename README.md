...
symbols = mt4.symbols()
signals = mt4.signals()
iac_indicator = mt4.indicator("iAC", ["EURUSD", 60, 1])
...
print(f"There are {len(signals)} signals, one of which is\n{next(iter(signals.items()))}\n")
print(f"Results for indicator iAC(EURUSD, 60, 1): {iac_indicator}")
```
```commandline
> I have 768.66 USD to trade against 173 symbols.
> 
> EURUSD symbol:
> Symbol(name="EURUSD, point_size=1e-05, digits=5, lot_size=100000.0, tick_value=1.0, tick_size=1e-05, min_lot=0.01, lot_step=0.01, max_lot=1000.0, margin_init=0.0, margin_maintenance=0.0, margin_hedged=0.0, margin_required=1177.15, freeze_level=0.0)
> 
> 	The latest tick is:
> 	SymbolTick(time=1596049440, bid=1.17708, ask=1.17715, last=0.0, volume=0)
> 
> 	The latest of 100 bars is:
> 	OHLCV(open="1.17694, high=1.17713, low=1.17665, close=1.17708, tick_volume=209, time=1596049200)
> 
...
> 
> Results for indicator iAC(EURUSD, 60, 1): 0.00025752
```
